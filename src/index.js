...
const SOLAR_PERSIST_INTERVAL_MS = 60000; // avoid hammering Gladys' config API on every MQTT update
...
async function reconfigure() {
  ...
  for (const sn of knownSerials) {
    ...
    const persistedWh = config ? Number(config[`solar_energy_wh_${sn}`]) : NaN;
...
async function maybePersistSolarAggregate() {
  ...
  for (const [sn, agg] of solarAggregateBySerial.entries()) {
    toPersist[`solar_energy_wh_${sn}`] = agg.totalWh;
  }
  if (Object.keys(toPersist).length === 0) return;

  try {
    await gladys.setConfig(toPersist);
  } catch (err) {
...
gladys.handleShutdown(async () => {
  ...
  // one extra API call on, to avoid losing up to a minute of accumulated
  // solar energy on every restart/redeploy.
  lastSolarPersistAtMs = 0;
  await maybePersistSolarAggregate().catch(() => {});

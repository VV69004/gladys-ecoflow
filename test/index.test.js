...
test("set_solar_energy_total persists the config key in lowercase, matching Gladys' real API validation", async () => {
  // CONFIRMED live (2026-09-12): the real Gladys API rejects config keys
  // that don't match /[a-z0-9_]/ with a GladysApiError, and this failed
  // SILENTLY on every single write for hours because the serial number is
  // uppercase (e.g. "R601ZEB4HE9T2333") — the mocked test environment
  // never caught this since it doesn't enforce that validation rule. This
  // test exists specifically so a future regression here is caught locally
  // instead of only in production.
  const state = await loadIndexWithMocks({
    config: { ...BASE_APP_LOGIN_CONFIG, serial_number: "R601ZEB4HE9T2333" },
  });

  await state.actions["set_solar_energy_total"]({ wh: 123 });

  const persistedKeys = Object.keys(state.setConfigCalls[state.setConfigCalls.length - 1]);
  for (const key of persistedKeys) {
    assert.match(key, /^[a-z0-9_]+$/, `config key "${key}" must be lowercase-only to pass Gladys' real API validation`);
  }
});
